...
---@class ElectricEngine : IComponent
ElectricEngine = class(IComponent)
...
function ElectricEngine.server_onFixedUpdate( self, timeStep )
	...
	self.interactable.publicData.consumptionE = self.data.consumptionE * (self.saved.gearIdx - 1) / (#Gears - 1)
	local multiply = 0
	if self.interactable.publicData.consumptionE > 0 then
		multiply = self.interactable.publicData.E / self.interactable.publicData.consumptionE
	end
	...
	for _, bearing in ipairs( bearings ) do
		bearing:setMotorVelocity( self.motorVelocity * multiply, self.motorImpulse * multiply)
	end
...
function ElectricEngine:server_onDestroy()
