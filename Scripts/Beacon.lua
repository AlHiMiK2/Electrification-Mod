...
Beacon = class(IComponent)
Beacon.maxParentCount = -1
Beacon.maxChildCount = 0
Beacon.connectionInput = sm.interactable.connectionType.logic
...
function Beacon.server_onCreate( self )
	...
	self.network:setClientData( { iconIndex = self.saved.iconData.iconIndex, colorIndex = self.saved.iconData.colorIndex }, 1 )
...
function Beacon:server_onFixedUpdate()
	if self.eCalculator == nil then
        IComponent.sv_register(self)
        return
    end
	self.interactable.active = true
	self.network:setClientData( {distance = self.interactable.publicData.E / self.data.consumptionE}, 2 )
end
...
function Beacon.client_onClientDataUpdate( self, clientData, channel)
	if channel == 1 then
		if self.cl == nil then
			self.cl = {}
		end
		...
		if g_beaconManager == nil then
			...
			self.cl.beaconIconGui:setMaxRenderDistance(10000 * (self.cl.distance or 0))
			self.cl.beaconIconGui:open()
		end
	elseif channel == 2 then
		self.cl.distance = clientData.distance
		print(self.cl.distance)
		if self.cl.beaconIconGui then
			self.cl.beaconIconGui:setMaxRenderDistance(10000 * self.cl.distance)
		end
	end
end
